...
class Renderer {
  ...
  renderObject(obj, camera) {
    ...
    if (!obj.program) {
      obj.setShaderProgram(this.gl, defaultVertexShader, defaultFragmentShader);
    }
    ...
    if (obj.type === 'cube') {
      ...
      let brightness = 1;
      for (let i = 0; i < obj.indices.length; i += 6) {
        this.gl.drawElements(drawType, 6, this.gl.UNSIGNED_BYTE, i);
